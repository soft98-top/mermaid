      ...
      if (svgElement) {
        try {
          ...
          allElements.forEach((el) => {
            ...
            // Handle node label references (existing logic)
          ...
          // Handle click events generated by Mermaid click statements
          // Check for both href and xlink:href attributes
          ...
          clickableElements.forEach((el) => {
            ...
            if (href && parsedDiagram.nestedDiagrams.has(href)) {
              // Add multiple event listeners to ensure interception
              const interceptClick = (event: Event) => {
                console.log('🔧 Intercepting click event for nested diagram:', href);
                event.preventDefault();
                event.stopPropagation();
              ...
              // Add listeners for different event phases
              el.addEventListener('click', interceptClick, true); // Capture phase
              el.addEventListener('click', interceptClick, false); // Bubble phase
              
              // Also handle mousedown to prevent any navigation
              el.addEventListener('mousedown', (event) => {
                event.preventDefault();
                event.stopPropagation();
              }, true);
              
              // Remove both href and xlink:href to prevent navigation
              el.removeAttribute('href');
              el.removeAttribute('xlink:href');
              
              // Also remove any onclick attributes that might exist
              ...
              if (el instanceof SVGElement || el instanceof HTMLElement) {
                ...
                // Ensure it's not treated as a link
                el.style.pointerEvents = 'auto';
              }
            }
          });

          // Add a global click interceptor for the entire SVG as a fallback
          svgElement.addEventListener('click', (event) => {
            const target = event.target as Element;
            if (target) {
              // Check if the clicked element or its parents have href or xlink:href attributes
              ...
              while (currentElement && currentElement !== svgElement) {
                ...
                if (href && parsedDiagram.nestedDiagrams.has(href)) {
                  console.log('🔧 Global interceptor caught click for nested diagram:', href);
                  event.preventDefault();
                  event.stopPropagation();
                  event.stopImmediatePropagation();
                  handleNestedInteraction(href, event as MouseEvent);
          ...
          // Set up a MutationObserver to watch for any href attributes being re-added
          const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
              if (mutation.type === 'attributes' && 
                ...
                if (href && parsedDiagram.nestedDiagrams.has(href)) {
                  console.log('🔧 MutationObserver: Removing re-added href/xlink:href:', href);
          ...
          // Start observing the SVG element for attribute changes
